...
const appointment_calendar_summary_1 = require("../src/utils/appointment-calendar-summary");
function assertEqual(actual, expected, label) {
    if (actual !== expected) {
        throw new Error(`${label}: expected "${String(expected)}", got "${String(actual)}"`);
    }
}
const summary = (0, appointment_calendar_summary_1.buildAppointmentCalendarSummary)([
